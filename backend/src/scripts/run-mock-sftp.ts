import { Server, SFTPWrapper } from 'ssh2';
...
// Thư mục lưu trữ local đóng vai trò đường dẫn /data/ trên SFTP
...
const server = new Server(
  ...
  (client) => {
    ...
    client.on('authentication', (ctx) => {
      console.log(`[SFTP Auth] Account attempting login: ${ctx.username}`);
      if (
    ...
    client.on('ready', () => {
      ...
      client.on('session', (accept) => {
        ...
        session.on('sftp', (accept) => {
          ...
          const sftp = accept();
          const openHandles = new Map<string, { type: 'dir' | 'file'; path: string; readEof?: boolean; fd?: number }>();
          ...
          function handleStat(reqId: number, p: string, sftpInstance: SFTPWrapper) {
            const localPath = resolveLocalPath(p);
            try {
              const stat = fs.statSync(localPath);
              sftpInstance.attrs(reqId, getSftpAttrs(stat));
            } catch (err) {
              sftpInstance.status(reqId, 2); // SSH_FX_NO_SUCH_FILE
            }
          }
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   SFTP Password:    (Chấp nhận mọi password)`);
  console.log(`   Remote Directory: /data/`);
  console.log(`   Local Directory:  ${SFTP_ROOT}`);
  console.log('====================================================');
});

function resolveLocalPath(remotePath: string): string {
...
function getSftpAttrs(stat: fs.Stats) {
  return {
