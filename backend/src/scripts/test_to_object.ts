...
async function test() {
  ...
  const job = await botJobModel.findOne({ jobType: 'FILE_AUDIT_CQG' }, {}, { sort: { createdAt: -1 } });
  if (!job) {
  ...
  console.log('job.payload.result:', (job.payload as any).result);

  console.log('\n--- Test 2: job.toObject() ---');
  const jobObj = job.toObject();
  console.log('Type of jobObj.payload:', typeof jobObj.payload, jobObj.payload.constructor.name);
  console.log('jobObj.payload.result:', jobObj.payload.result);
  console.log('jobObj.payload.result.isWaitingFiles:', jobObj.payload.result?.isWaitingFiles);
