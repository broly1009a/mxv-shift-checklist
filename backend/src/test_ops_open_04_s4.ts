...
async function main() {
  ...
  const activeShift = await shiftLogModel.findOne({ status: { $ne: 'COMPLETED' } }).exec();
  if (!activeShift) {
  ...
  console.log(`Active Shift Log: ${activeShift._id} date=${activeShift.shiftDate}`);
  
  let task = activeShift.details.find((d: any) => d.taskId === 'ops_open_04_s4');
  if (!task) {
  ...
  const updatedTask = updatedShift.details.find((d: any) => d.taskId === 'ops_open_04_s4');
