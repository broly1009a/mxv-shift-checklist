...
async function runTests() {
  ...
  console.log(
    'Clearing existing templates in MongoDB to force a fresh seed...',
  );
  await templateModel.deleteMany({}).exec();

  console.log(
    'Re-running SeedService to populate templates with dependsOnTaskIds...',
  );
  ...
  const template = await templateModel
    .findOne({ title: 'Checklist Mở Cửa - IT Vận Hành Core' })
    .exec();
  if (!template) {
  ...
  for (const t of template.tasks) {
    console.log(
      `Task: ${t.taskId}, Name: ${t.taskName}, dependsOnTaskIds:`,
      JSON.stringify(t.dependsOnTaskIds),
    );
  }
  console.log('------------------------------------------');
  ...
  for (const t of shiftLog.details) {
    console.log(
      `Task: ${t.taskId}, Name: ${t.taskNameSnapshot}, dependsOnTaskIdsSnapshot:`,
      JSON.stringify(t.dependsOnTaskIdsSnapshot),
    );
  }
  console.log('------------------------------------');
  ...
  try {
    console.log(
      'Test 1: Attempting to complete it_open_04 while it_open_01 is still PENDING...',
    );
    try {
      ...
      throw new Error(
        'Test 1 FAILED: Allowed checking it_open_04 without its dependency completed!',
      );
    } catch (err) {
    ...
    const updatedLogAfterCheck = await shiftsService.updateTaskStatus(
      shiftLog._id.toString(),
      'it_open_04',
      'PASSED',
      staffUser,
    );
    const itOpen04Task = updatedLogAfterCheck.details.find(
      (d) => d.taskId === 'it_open_04',
    );
    if (
      itOpen04Task &&
      itOpen04Task.isChecked &&
      ...
      throw new Error(
        'Test 2 FAILED: Task it_open_04 was not marked completed.',
      );
    }

    console.log(
      'Test 3: Attempting to reset parent task it_open_01 back to PENDING while it_open_04 is still PASSED...',
    );
    try {
      await shiftsService.updateTaskStatus(
        shiftLog._id.toString(),
      ...
      throw new Error(
        'Test 3 FAILED: Allowed unchecking parent task while child task is still checked!',
      );
    } catch (err) {
      if (err instanceof BadRequestException) {
        console.log(
    ...
    console.log(
      'Test 4: Resetting dependent task it_open_04 back to PENDING...',
    );
    const resetLog = await shiftsService.updateTaskStatus(
      shiftLog._id.toString(),
      'it_open_04',
      'PENDING',
      staffUser,
    ...
    if (
      ...
      throw new Error(
        'Test 4 FAILED: Timestamps or status were not correctly reset/cleared.',
      );
    }
    ...
    const closeTemplate = await templateModel
      .findOne({ title: 'Checklist Đóng Cửa - IT Vận Hành Core' })
      .exec();
    if (closeTemplate) {
      console.log('Initializing closing shift log...');
      const closeShiftLog = await shiftsService.initializeShift(
        closeTemplate._id.toString(),
        adminUser,
        shiftDate,
      );

      console.log(
        'Injecting a CE/ACM backup task to test the closeShift guard...',
      );
      closeShiftLog.details[0].taskId = 'it_close_ce_backup';
      closeShiftLog.details[0].taskNameSnapshot =
        'Thực hiện tiến trình sao lưu cơ sở dữ liệu tự động CE';
      await closeShiftLog.save();

      console.log('--- Closing Shift Log Details ---');
      console.log(
        'sessionType:',
        (closeShiftLog.templateId as any)?.sessionType,
      );
      for (const d of closeShiftLog.details) {
        const containsCeOrAcm =
          d.taskId.toLowerCase().includes('ce') ||
          d.taskId.toLowerCase().includes('acm');
        const containsBackupOrSaoLuu =
          d.taskNameSnapshot.toLowerCase().includes('backup') ||
          d.taskNameSnapshot.toLowerCase().includes('sao lưu');
        console.log(
          `Task: ${d.taskId}, Name: ${d.taskNameSnapshot}, containsCeOrAcm: ${containsCeOrAcm}, containsBackupOrSaoLuu: ${containsBackupOrSaoLuu}`,
        );
      }

      console.log(
        'Attempting to close the shift with incomplete backup tasks...',
      );
      try {
        await shiftsService.closeShift(
          closeShiftLog._id.toString(),
          adminUser,
          'Test handover',
        );
        throw new Error(
          'Test 5 FAILED: Allowed closing shift with incomplete CE/ACM backup tasks!',
        );
      } catch (err) {
        if (err instanceof BadRequestException) {
          console.log(
