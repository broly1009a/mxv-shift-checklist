...
async function main() {
  ...
  for (let r = 2; r <= ws.rowCount; r++) {
    ...
    if (maTKGD.includes('SI5CO') || maHD.includes('SI5CO') || maTKGD.includes('PL1NY') || maHD.includes('PL1NY') || maTKGD.includes('CP2CO') || maHD.includes('CP2CO')) {
      console.log(`Row ${r}: maTKGD=${maTKGD}, maHD=${maHD}, qty=${ws.getCell(r, 13).value}`);
    }
  }
}
