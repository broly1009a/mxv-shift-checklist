...
async function main() {
  ...
  const payload = {
    ngayGD: '2026-07-16',
    macroPath: 'c:\\Users\\hiepth\\OneDrive - MERCANTILE EXCHANGE OF VIETNAM\\Documents\\Github\\mxv-shift-checklist\\marco\\Thong ke gia tri giao dich có ACM\\Macro thong ke gia tri giao dich có ACM.xlsm',
    targetRoot: baseDir,
    ...
    pathOptions: path.join(baseDir, 'Thong ke gia tri giao dich Options 2026.xlsx'),
    pathSpread: path.join(baseDir, 'Thong ke gia tri giao dich Spread 2026.xlsx'),
  };
  ...
  const reportPath = path.join(reportDir, 'bot-check-value-report-20260716.json');
