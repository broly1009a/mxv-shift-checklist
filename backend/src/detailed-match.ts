...
// 4. Perform Match and print details
console.log('--- MATCHING DETAILS ---');
const matchedByContract = new Map<string, Set<string>>();
  ...
  for (const pos of aggregatedPos.values()) {
    if (pos.symbol.toUpperCase() === c.contractCode.toUpperCase()) {
      if (c.side === 'BUY' && pos.buyVol > 0) {
        matchSet.add(`${pos.account} (Open BUY: ${pos.buyVol})`);
      } else if (c.side === 'SELL' && pos.sellVol > 0) {
        matchSet.add(`${pos.account} (Open SELL: ${pos.sellVol})`);
      }
    }
  }
...
for (const [contractInfo, accounts] of matchedByContract.entries()) {
  console.log(`\nContract: ${contractInfo}`);
  console.log(`Matched accounts (${accounts.size}):`);
