...
async function runTest() {
  ...
  const workspaceRoot = 'c:\\Users\\hiepth\\OneDrive - MERCANTILE EXCHANGE OF VIETNAM\\Documents\\Github\\mxv-shift-checklist';
  const macroPath = path.join(workspaceRoot, 'marco', 'Thong ke gia tri giao dich có ACM', 'Macro thong ke gia tri giao dich có ACM.xlsm');
  ...
  const createDummyTracker = async (filename: string, commodities: string[]) => {
  ...
  const pathNormal = await createDummyTracker('Thong ke gia tri giao dich 2026.xlsx', NORMAL_COMMODITIES);
  const pathSpread = await createDummyTracker('Thong ke gia tri giao dich Spread 2026.xlsx', SPREAD_COMMODITIES);
  const pathLme = await createDummyTracker('Thong ke gia tri giao dich LME 2026.xlsx', LME_COMMODITIES);
  const pathOptions = await createDummyTracker('Thong ke gia tri giao dich Options 2026.xlsx', OPTIONS_COMMODITIES);
  const pathAcm = await createDummyTracker('Thong ke gia tri giao dich ACM 2026.xlsx', ACM_COMMODITIES);
  ...
  setupDir(path.join(testRoot, 'Backup MS', 'Futures', '2026', 'T07.2026', '14.07'));

  // Move / Copy the initialized files to their structure paths
  fs.copyFileSync(pathNormal, path.join(testRoot, 'Thong ke gia tri giao dich', 'Thong ke gia tri giao dich 2026.xlsx'));
  fs.copyFileSync(pathSpread, path.join(testRoot, 'Backup MS', 'Spread', '2026', 'Thong ke gia tri giao dich Spread 2026.xlsx'));
  fs.copyFileSync(pathLme, path.join(testRoot, 'Backup CQG', 'LME', '2026', 'Thong ke gia tri giao dich LME 2026.xlsx'));
  fs.copyFileSync(pathOptions, path.join(testRoot, 'Thong ke gia tri giao dich', 'Thong ke gia tri giao dich Options 2026.xlsx'));
  fs.copyFileSync(pathAcm, path.join(testRoot, 'Thong ke gia tri giao dich', 'Thong ke gia tri giao dich ACM 2026.xlsx'));

  // Copy DSGD.xlsx to test structure
  fs.copyFileSync(dsgdPath, path.join(testRoot, 'Backup MS', 'Futures', '2026', 'T07.2026', '14.07', 'DSGD.xlsx'));
  ...
  for (let r = 11; r <= 77; r++) {
    ...
    const val = typeof valCell.value === 'object' && valCell.value !== null && 'result' in valCell.value 
  ...
  const spreadSpOrder = ['ZSE', 'C.ZCE', 'ZLE', 'ZCE', 'P.ZCE', 'ZRE', 'XC', 'TRU', 'MHG'];
  ...
  const verifyTracker = async (filePath: string, commodities: string[], expectedMap: Map<string, number>, label: string) => {
    const wb = new ExcelJS.Workbook();
    ...
    for (let r = 5; r <= ws.rowCount; r++) {
      ...
      if (dateObj && !isNaN(dateObj.getTime()) && dateObj.toISOString().startsWith('2026-07-14')) {
        targetRowIndex = r;
        break;
      }
    }
    ...
    if (targetRowIndex === -1) {
      console.error(`❌ Lỗi: Không tìm thấy dòng cho ngày 2026-07-14 trong file ${label}`);
      mismatchCount++;
      return;
    }
    ...
    for (let i = 0; i < commodities.length; i++) {
      ...
      if (diff > 0.01) {
        console.error(`❌ BẤT ĐỒNG: [${code}] Lũy kế=${writtenVal.toLocaleString()} vs Kỳ vọng=${expectedVal.toLocaleString()} (Lệch: ${diff.toLocaleString()})`);
        mismatchCount++;
      } else {
  ...
  await verifyTracker(
    path.join(testRoot, 'Thong ke gia tri giao dich', 'Thong ke gia tri giao dich 2026.xlsx'),
    NORMAL_COMMODITIES,
    expectedNormal,
    'Normal (L11:L77)'
  );
  ...
  await verifyTracker(
    path.join(testRoot, 'Backup MS', 'Spread', '2026', 'Thong ke gia tri giao dich Spread 2026.xlsx'),
    SPREAD_COMMODITIES,
    expectedSpread,
    'Spread (N12:N20)'
  );
  ...
  // Verify LME Tracker
  await verifyTracker(
    path.join(testRoot, 'Backup CQG', 'LME', '2026', 'Thong ke gia tri giao dich LME 2026.xlsx'),
    LME_COMMODITIES,
  ...
  await verifyTracker(
    path.join(testRoot, 'Thong ke gia tri giao dich', 'Thong ke gia tri giao dich Options 2026.xlsx'),
    OPTIONS_COMMODITIES,
  ...
  // Verify ACM Tracker
  await verifyTracker(
    path.join(testRoot, 'Thong ke gia tri giao dich', 'Thong ke gia tri giao dich ACM 2026.xlsx'),
    ACM_COMMODITIES,
  ...
  if (mismatchCount === 0) {
    ...
    console.error(`❌ THẤT BẠI: Có ${mismatchCount} lỗi lệch giá trị đối chiếu.`);
  }
  console.log('================================================');
}
