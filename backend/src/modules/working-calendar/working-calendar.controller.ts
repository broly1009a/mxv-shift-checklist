...
export class WorkingCalendarController {
  ...
  async create(@Body() body: any, @Request() req: any) {
    return this.workingCalendarService.create(body, req.user._id || req.user.id);
  }

  @UseGuards(RolesGuard)
  @Roles('ADMIN')
  @Put(':date')
  async update(@Param('date') date: string, @Body() body: any, @Request() req: any) {
    return this.workingCalendarService.update(date, body, req.user._id || req.user.id);
  }
