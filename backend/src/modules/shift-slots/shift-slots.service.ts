...
export class ShiftSlotsService {
  ...
  async create(data: any): Promise<ShiftSlot> {
    const existing = await this.shiftSlotModel.findOne({ code: data.code }).exec();
    if (existing) {
      throw new ConflictException(`Shift slot code ${data.code} already exists`);
    }
  ...
  async update(id: string, data: any): Promise<ShiftSlot> {
    if (data.code) {
      const existing = await this.shiftSlotModel.findOne({ code: data.code, _id: { $ne: id } }).exec();
      if (existing) {
        throw new ConflictException(`Shift slot code ${data.code} already exists`);
      }
    }
    const updated = await this.shiftSlotModel.findByIdAndUpdate(id, data, { new: true }).exec();
    if (!updated) {
      throw new NotFoundException(`Shift slot with ID ${id} not found`);
    }
    return updated;
