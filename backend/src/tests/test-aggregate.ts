...
console.log(
  `Contracts with deadline <= 15/07/2026:`,
  filteredContracts.map(
    (c) => `${c.contractCode} (${c.side}) - Deadline: ${c.deadline}`,
  ),
);
  ...
  for (const ord of aggregatedOrders.values()) {
    if (ord.symbol.toUpperCase() === c.contractCode.toUpperCase()) {
      const existing = results.find(
        (r) => r.account === ord.account && r.symbol === ord.symbol,
      );
      if (existing) {
        ...
        results.push({
          ...
          side: c.side,
          openVol: 0,
          pendingVol: ord.buyPending + ord.sellPending,
        });
