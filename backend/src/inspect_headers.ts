...
async function inspect() {
  ...
  const files = [
    {
      gen: path.join(baseDir, 'Thong ke so lot giao dich ACM 2026 2.xlsx'),
      root: path.join(
        rootDir,
        'Thong ke so lot giao dich ACM 2026 2 root.xlsx',
      ),
    },
  ];
  ...
  for (const f of files) {
    ...
    for (let c = 1; c <= Math.max(wsGen.columnCount, wsRoot.columnCount); c++) {
      ...
      if (vGen !== null || vRoot !== null) {
        console.log(
          `Col ${c}: HeaderGen=${JSON.stringify(hGen)}, HeaderRoot=${JSON.stringify(hRoot)} | Gen=${JSON.stringify(vGen)}, Root=${JSON.stringify(vRoot)}`,
        );
      }
    }
  }
