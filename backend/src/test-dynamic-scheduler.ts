...
async function runTests() {
  ...
  const templateModel = app.get<Model<ChecklistTemplate>>(getModelToken(ChecklistTemplate.name));
  ...
  const seededConfigRaw = await settingsService.getSetting('bot_scheduler_config', '');
  if (seededConfigRaw) {
    ...
    parsed.forEach((t: any) => console.log(` - ID: ${t.id}, JobType: ${t.jobType}, Scheduled Time: ${t.time}, Enabled: ${t.enabled}`));
  } else {
    throw new Error('TEST 1 FAILED: bot_scheduler_config not found or empty in database.');
  }
  ...
  console.log('Creating a temporary template containing an AUTO_CHECK_SOD task...');
  ...
  const shiftDate = new Date(Date.now() + 7 * 60 * 60 * 1000).toISOString().split('T')[0];
  ...
  try {
    ...
    shiftLog = await shiftsService.initializeShift(savedTemplate._id.toString(), adminUser, shiftDate);
    console.log(`Shift log initialized. ID: ${shiftLog._id}`);

    // Verify task details
    const targetTask = shiftLog.details.find(d => d.taskId === 'temp_check_sod');
    if (!targetTask) {
      throw new Error('Failed to initialize task temp_check_sod in shift log');
    }
    console.log(`Target task found: ${targetTask.taskNameSnapshot}, botCheckTypeSnapshot: ${targetTask.botCheckTypeSnapshot}`);
    ...
    console.log(`Current Vietnam Time (GMT+7) calculated: ${currentTimeStr} (${todayStr})`);

    // Backup current scheduler configuration
    backupSchedulerConfig = await settingsService.getSetting('bot_scheduler_config', '[]');

    // Inject temporary scheduler config targeting the exact current time
    console.log(`Injecting temporary scheduler configuration for current time: ${currentTimeStr}...`);
    const tempSchedules = [
      {
    ...
    await settingsService.setSetting('bot_scheduler_config', JSON.stringify(tempSchedules, null, 2));
    ...
    const enqueuedJob = await botJobModel.findOne({
      jobType: 'AUTO_CHECK_SOD',
    ...
    if (enqueuedJob) {
      const jobPayload = enqueuedJob.payload instanceof Map
      ...
      throw new Error('TEST 2 FAILED: Job was not enqueued or not linked correctly to shift log task!');
    }

    // Verify duplicate prevention (lastRunMap)
    console.log('Checking lastRunMap duplicate execution guard...');
    const lastRunDate = (schedulerService as any).lastRunMap.get('TEST_AUTO_CHECK_SOD');
    if (lastRunDate === todayStr) {
      console.log('✅ lastRunMap populated correctly with today\'s date:', lastRunDate);
    } else {
      throw new Error('TEST 2 FAILED: lastRunMap was not updated for the executed scheduled task!');
    }

    // Triggering checkSchedule again should NOT enqueue another job (duplicate prevention)
    console.log('Triggering checkSchedule again to verify duplicate execution prevention...');
    const initialJobsCount = await botJobModel.countDocuments({
      jobType: 'AUTO_CHECK_SOD',
      'payload.taskId': 'temp_check_sod',
      'payload.shiftLogId': shiftLog._id.toString(),
    }).exec();
    ...
    const postJobsCount = await botJobModel.countDocuments({
      jobType: 'AUTO_CHECK_SOD',
      'payload.taskId': 'temp_check_sod',
      'payload.shiftLogId': shiftLog._id.toString(),
    }).exec();
    ...
    if (initialJobsCount === postJobsCount) {
      console.log('✅ Duplicate execution blocked successfully! No additional jobs enqueued.');
    } else {
      throw new Error('TEST 2 FAILED: Duplicate execution was allowed, enqueued another job!');
    }
    ...
    if (backupSchedulerConfig !== null) {
      await settingsService.setSetting('bot_scheduler_config', backupSchedulerConfig);
      console.log('Restored original scheduler config in DB.');
    }
    // Delete any created bot job
    if (shiftLog) {
      await botJobModel.deleteMany({ 'payload.shiftLogId': (shiftLog as any)._id.toString() }).exec();
