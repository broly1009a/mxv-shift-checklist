...
import { Folder, Sparkles, Calendar, Info, CheckCircle2, AlertCircle, Loader2 } from 'lucide-react';
 ...
 * Không hiển thị đường dẫn kỹ thuật Linux hay chữ Ubuntu lên giao diện.
 */
...
export default function SmartPathInput({
  ...
  placeholder = 'vd: M:\\Tailieuchung\\QLGD-IT\\... hoặc dán C:\\Downloads\\...',
  style,
}: SmartPathInputProps) {
  const { token } = useAuth();
    ...
    <div style={style}>
      ...
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
        ...
        <button
          ...
          title="Kiểm tra xem thư mục có tồn tại và hệ thống có quyền ghi dữ liệu hay không"
        >
          {isVerifying ? (
            <>
            ...
            <>
              ...
              <span>Kiểm tra quyền ghi</span>
            </>
          )}
        </button>
      </div>

      {/* Hiển thị kết quả kiểm tra quyền ghi thư mục */}
      {verifyResult && (
        <div
          style={{
            marginTop: '8px',
          ...
          <div>
            ...
            {verifyResult.resolvedPath && (
              <div style={{ fontSize: '0.68rem', opacity: 0.85, marginTop: '2px', fontFamily: 'monospace' }}>
                Đường dẫn trên hệ thống: {verifyResult.resolvedPath}
              </div>
            )}
          </div>
        </div>
      ...
      {/* Hộp gợi ý chuyển đổi thông minh khi dán đường dẫn cá nhân (C:\) hoặc đường dẫn chưa chuẩn */}
      {parsed && parsed.hasConversionDiff && (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#2563eb', marginBottom: '8px' }}>
            <Info size={14} />
            <span style={{ fontWeight: 600 }}>Đề xuất chuẩn hóa sang thư mục chia sẻ mạng (ổ M:):</span>
          </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '10px' }}>
            {parsed.hasDynamicVars && (
              <button
                ...
                title="Tự động cập nhật thư mục theo từng ngày trực"
              >
                <Sparkles size={13} /> Áp dụng đường dẫn động theo ngày (${'YYYY'}\\${'DD'}.${'MM'})
              </button>
            )}
            ...
            >
              <Folder size={13} /> Áp dụng đường dẫn ổ M cố định
            </button>
          </div>

          {/* Xem trước thư mục thực tế sẽ lưu file */}
      ...
      {parsed && !parsed.hasConversionDiff && parsed.hasDynamicVars && (
        <div style={{ marginTop: '6px', fontSize: '0.72rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          <span>
            <strong style={{ color: '#059669' }}>Xem trước:</strong> Hôm nay lưu về:{' '}
