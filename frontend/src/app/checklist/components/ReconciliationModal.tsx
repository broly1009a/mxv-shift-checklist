...
import { X, FileSpreadsheet, Play, AlertTriangle, CheckCircle2, Info, ArrowRight } from 'lucide-react';
...
export default function ReconciliationModal({
  ...
  const handleRunReconciliation = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/reconciliation/upload-klgd`, {
        method: 'POST',
        headers: {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h2 style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--text-primary)', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            Đối Chiếu Dữ Liệu Excel Tự Động
          </h2>
          <button 
            onClick={onClose} 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end' }}>
              <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', lineHeight: '1.4', display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(59,130,246,0.05)', padding: '10px', borderRadius: '8px', border: '1px solid rgba(59,130,246,0.1)' }}>
                ...
                Hệ thống sẽ đối chiếu khối lượng giao dịch khớp lệnh và trạng thái mở (TTM) tự động dựa trên cấu trúc các cột tương ứng.
              </span>
            </div>
          </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.75rem', textAlign: 'left' }}>
                      <thead style={{ background: 'rgba(128,128,128,0.05)', position: 'sticky', top: 0 }}>
                        <tr>
                          ...
                          <th style={{ padding: '8px', borderBottom: '1px solid var(--border-color)' }}>Mã lệnh / Ord</th>
